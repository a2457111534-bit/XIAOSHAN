"""引擎逻辑测试（假键盘注入，不真打字）：.venv/bin/python tests/test_engine.py"""
import pathlib
import sys

sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent))
from voice_typist import axtext
from voice_typist import inject
from voice_typist.commands import Cmd
from voice_typist.engine import Engine

axtext.read_focused = lambda: (None, None, None)   # 测试默认读不到，走账本逻辑
axtext.write_focused = lambda t: False              # 测试默认AX不可写，走退格回退路径

BK, RET, Z = 0x33, 0x24, 0x06
typed, taps, clips, pastes = [], [], [], []

inject.type_text = lambda text, **kw: typed.append(text) or True
inject.tap_key = lambda code, **kw: taps.append(
    (code, kw.get("repeat", 1), kw.get("cmd", False), kw.get("shift", False))) or True
inject.set_clipboard_text = lambda text, **kw: clips.append(text)
inject.paste_clipboard = lambda **kw: pastes.append(True)
...
check("替换:缓冲", eng.session.text == A2, eng.session.text)
...
check("撤销:发了Cmd+Z", any(t[0] == Z and t[2] for t in taps), taps)
...
eng.execute(Cmd("replace", old="快速推进", new="飞速推进"))
_writes_undo = []
axtext.read_focused = lambda: (eng.session.text, len(eng.session.text), 0)
axtext.write_focused = lambda t: _writes_undo.append(t) or True
taps.clear()
ok, msg = eng.execute(Cmd("undo"))
...
engU = Engine()
axtext.read_focused = lambda: (engU.session.text, len(engU.session.text), 0)
axtext.write_focused = lambda t: False
engU.execute_dictation("第一句。")
engU.execute_dictation("第二句。")
engU.execute(Cmd("delete_last_sentence"))
taps.clear(); clips.clear(); pastes.clear()
ok, msg = engU.execute(Cmd("undo"))
check("撤回:AX不可写全选重打", ok and any(t[0] == 0x00 and t[2] for t in taps)
      and clips == ["第一句。第二句。"] and pastes == [True], (taps, clips))
axtext.read_focused = lambda: (None, None, None)
axtext.read_focused = lambda: (None, None, None)
axtext.write_focused = lambda t: False
...
# AX对齐：账本与输入框不一致时，删除前按输入框真实内容重置（含删除后复核）
eng8 = Engine()
...
axtext.read_focused = _fake_read
taps.clear(); clips.clear(); pastes.clear()
ok, msg = eng8.execute(Cmd("delete_last_sentence"))
check("AX对齐:按真实内容删", ok and eng8.session.text == "旧账本。",
      (msg, eng8.session.text))
check("AX对齐:退格按真实长度", bk_taps()[0][1] == 7, taps)
axtext.read_focused = lambda: ("文字", 2, 2)
ok, msg = eng8.execute(Cmd("delete_last_sentence"))
check("有选中文字拒绝", ok is False and "选中" in msg, msg)
axtext.read_focused = lambda: ("文字", 1, 0)
ok, msg = eng8.execute(Cmd("delete_last_sentence"))
...
# AX原子写入路径：删除不发任何键盘事件
writes = []
axtext.write_focused = lambda t: writes.append(t) or True
eng9 = Engine()
eng9.execute_dictation("你好啊。你是谁呀？")
taps.clear(); pastes.clear()
ok, msg = eng9.execute(Cmd("delete_last_of", sub="你是谁呀"))
check("AX写入:零键盘事件", taps == [] and pastes == [], (taps, pastes))
check("AX写入:整框原子设值", writes == ["你好啊。？"], writes)
check("AX写入:缓冲同步", eng9.session.text == "你好啊。？", eng9.session.text)

# 删除全部（AX路径 + 回退路径）
eng9.execute_dictation("第一句。第二句。")
writes.clear()
ok, msg = eng9.execute(Cmd("delete_all"))
check("删全部:AX原子清空", ok and writes == [""] and eng9.session.text == "", (msg, writes))
axtext.write_focused = lambda t: False
eng9.execute_dictation("第三句。")
taps.clear()
ok, msg = eng9.execute(Cmd("delete_all"))
check("删全部:回退⌘A+退格", ok and any(t[0] == 0x00 and t[2] for t in taps)
...
eng10.execute_dictation("稳步推进各项任务。")
writes.clear()
ok, msg = eng10.execute(Cmd("delete_containing", sub="文布推进"))  # '稳步'被听成'文布'
check("同音兜底:含句删", ok and eng10.session.text == "今天天气很。他来到了上海。",
      eng10.session.text)
