"""命令执行：把解析出的命令变成键盘事件 + 会话缓冲区修改。

所有修改都遵循同一套路：退格到目标位置 → 重打需要保留的尾部 → 同步缓冲区。
"""
import threading

from . import axtext
from . import inject
from .commands import PUNCT_CHARS
...
class Engine:
    ...
    def _execute_locked(self, cmd):
        ...
        if k not in ("newline", "paragraph", "undo", "redo", "send"):
            try:
                ...
                if text is not None:
                    ...
                    if text != ses.text:
                        _log(f"AX对齐: 输入框{len(text)}字/账本{len(ses.text)}字，已按实际重置")
                        ses.set_full_text(text)
                else:
                    _log("AX读不到焦点文本框，按账本兜底")
            except Exception:
        ...
        if k == "delete_all":
            ...
            if axtext.write_focused(""):   # 原子清空，零键盘事件
                ses.set_full_text("")
                _log(f"全部清空(AX原子写入, 原{len(text)}字)")
                return True, "已全部清空"
            _log(f"全部清空(回退⌘A+退格, {len(text)}字)")
        ...
        if k == "insert":
            ...
            inject.set_clipboard_text(cmd.content + ses.text[pos:])
            inject.paste_clipboard()
            ses.set_full_text(new_text)
            return True, f"已加上{cmd.content}"
        if k == "insert_append":
            ses.snapshot()
            inject.set_clipboard_text(cmd.content)
            inject.paste_clipboard()
            ses.add(cmd.content)
            return True, f"已加上{cmd.content}"
        if k == "delete_batch":
        ...
        if k == "delete_n_of":
            ...
            for _ in range(cmd.n):
                ...
                for v in variants:
                    ...
                    if i >= 0:
                        ...
                        break
                if sp is None:
                    from .session import fuzzy_rfind
            ...
            if axtext.write_focused(buf):
                ...
                _log(f"删N个[{cmd.sub}×{cmd.n}] AX原子写入 → 剩{len(buf)}字")
    ...
    def _delete_span(self, a, b, desc):
        ...
        if axtext.write_focused(new_text):   # AX原子写入：零键盘事件，不可能误删
            self.session.set_full_text(new_text)
            _log(f"删除[{a}:{b}] AX原子写入 → 剩{len(new_text)}字")
            return True, desc
        _log(f"删除[{a}:{b}] AX不可写，回退退格{n_back}次+粘贴{len(tail)}字")
        inject.tap_key(inject.VK_BACKSPACE, repeat=n_back, delay=0.002)
        if tail:
            inject.set_clipboard_text(tail)
            inject.paste_clipboard()
    ...
    def _replace_span(self, a, b, new):
        ...
        if axtext.write_focused(new_text):
            self.session.set_full_text(new_text)
            _log(f"替换[{a}:{b}] AX原子写入 → {len(new_text)}字")
            return True, "已修改"
        _log(f"替换[{a}:{b}] AX不可写，回退退格{n_back}次+粘贴")
        inject.tap_key(inject.VK_BACKSPACE, repeat=n_back, delay=0.002)
        inject.set_clipboard_text(new + total[b:])
        inject.paste_clipboard()
    ...
    def _post_verify(self):
        """删除/替换后复核：再读一次输入框，与账本比对，不一致当场报警。"""
        try:
            import time as _t
            _t.sleep(0.15)   # 等粘贴落地
            text, _, _ = axtext.read_focused()
